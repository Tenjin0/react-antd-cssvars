...
const ColorPickerChooser: React.FunctionComponent<IColorPicker> = props => {
	const [colorPickerState, setColorPickerState] = useState<IColorPickerState>({ open: false, color: props.default })

	const onClickButton = (event) => {
		setColorPickerState({ ...colorPickerState, open: !colorPickerState.open })
	}

	const onChangeColor = (color: ColorResult, event) => {
		if (colorPickerState.color !== color.hex) {
			setColorPickerState({ ...colorPickerState, color: color.hex, open: !colorPickerState.open })
			if (props.onChange) {
		...
		<div className="color-picker-chooser">
			<Button type="primary" onClick={onClickButton}>Primary Button</Button>
			{
				...
				<div className="color-picker-container">
					...
					<Button className="color-picker-btn-close" shape="circle" size="small" type="text" onClick={onClose}><CloseOutlined /></Button>
				</div>
			}
